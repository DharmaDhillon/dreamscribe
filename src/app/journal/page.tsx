...
function getDateString() {
  ...
  return `${days[now.getDay()]}, the ${nth} of ${months[now.getMonth()]} · ${now.getFullYear()}`;
}
...
export default function JournalPage() {
  ...
  }, [isRecording, startRecording, stopRecording, userId]);
  ...
  const handleSaveAndAnalyze = async (blob: Blob) => {
    ...
    try {
      ...
      // Type out Claude insight
      setProcessing(false);
      setShowInsight(true);
      setQuillWriting(true);

      // Build the full entry for display
    ...
    <>
      ...
      <section className="pt-[130px] text-center max-w-[700px] mb-8">
        <h1
          ...
          style={{
            textShadow: "0 0 30px rgba(232,168,74,0.5), 0 0 60px rgba(201,124,42,0.3)",
            opacity: 0,
      ...
      {/* Main scroll */}
      <div style={{ opacity: 0, animation: "ember-in 2s ease forwards 0.8s" }}>
        <ScrollContainer>
          {/* Ornament */}
          ...
          <div className="relative z-[2] flex justify-center gap-1 mb-4">
            {(["dream", "daily"] as EntryType[]).map((type) => (
              <button
                ...
                className={`px-6 py-2.5 font-fell italic text-[0.75rem] tracking-[0.1em] transition-all duration-300 border-b-2 ${
                  entryType === type
                    ? "text-ink-sepia opacity-80 border-amber shadow-[0_2px_8px_rgba(201,124,42,0.3)]"
                    : "text-ink-sepia opacity-40 border-transparent hover:opacity-60"
                }`}
                style={{
                  cursor: "none",
          ...
          <div className="relative z-[2] flex justify-center mb-8">
            <button
              ...
              className="font-fell italic text-[0.65rem] tracking-[0.1em] px-4 py-1.5 transition-all duration-300 border"
              style={{
                cursor: "none",
                background: isPublic ? "rgba(201,124,42,0.1)" : "rgba(44,24,16,0.05)",
                borderColor: isPublic ? "rgba(201,124,42,0.3)" : "rgba(139,100,40,0.2)",
                color: "rgba(44,24,16,0.6)",
              }}
            >
              {isPublic ? "🔓 public · visible on feed" : "🔒 private · only you"}
            </button>
          </div>
          ...
          <div className="relative z-[2] min-h-[180px]">
            ...
            <div className="font-pinyon text-[1.5rem] text-ink-sepia leading-[2.1] min-h-[140px] relative">
              ...
              {isRecording && (
                <span className="opacity-60">Recording your voice...</span>
              )}
          ...
          {!processing && (
            <div className="relative z-[2] flex flex-col items-center gap-4 mt-6">
              <div className="flex items-center gap-4 w-full mb-2">
                ...
                <div className="font-fell italic text-[0.7rem] text-ink-sepia opacity-50 tracking-[0.1em] whitespace-nowrap">
                  {isRecording ? "press the seal to stop" : "press the seal to speak your truth"}
                </div>
                <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[rgba(139,100,40,0.3)] to-transparent" />
              </div>
              ...
              >
                ...
                <div className="seal-wax">
                  <span
                    className="font-pinyon text-[1.6rem] text-[rgba(255,220,180,0.7)]"
              ...
              {/* Timer */}
              <div className={`font-fell italic text-base text-ink-sepia tracking-[0.1em] transition-opacity duration-300 ${isRecording ? "opacity-70" : "opacity-0"}`}>
                {formatTime(elapsedSeconds)}
              </div>

              <div className="font-fell italic text-[0.65rem] text-ink-sepia tracking-[0.15em] opacity-55">
                {isRecording ? "speaking..." : "seal of truth"}
              </div>

              {micError && (
                <p className="font-cormorant italic text-sm text-[#8b1a1a] opacity-80 text-center mt-2 max-w-xs">{micError}</p>
              )}
              {savingError && (
                <p className="font-cormorant italic text-sm text-ink-sepia opacity-60 text-center mt-4 max-w-sm leading-relaxed">{savingError}</p>
              )}
            </div>
          )}
          ...
          {processing && (
            <div className="flex flex-col items-center gap-5 text-center relative z-[2]">
              ...
              <div className="font-pinyon text-[1.8rem] text-ink-sepia opacity-80">
              ...
              <div className="flex flex-col gap-2 text-left">
                {PROC_STEP_LABELS.map((label, i) => (
                  <div
                    key={i}
                    className={`font-fell italic text-[0.75rem] text-ink-sepia flex items-center gap-2 tracking-[0.05em] transition-opacity duration-500 ${
                      processingSteps[i] ? "opacity-80" : "opacity-30"
                    }`}
                  >
          ...
          {/* After insight — link to feed */}
          {showInsight && !quillWriting && (
            <div className="relative z-[2] text-center mt-8 pt-6 border-t border-[rgba(139,100,40,0.15)]">
              <Link
                href="/feed"
                className="font-cormorant italic text-sm text-ink-sepia opacity-50 hover:opacity-80 transition-opacity no-underline"
                style={{ cursor: "none" }}
              >
                View all your scrolls in the feed &rarr;
              </Link>
            </div>
          )}
        </ScrollContainer>
        ...
        >
          <div className="text-center mb-6">
            <div className="flex items-center gap-4 justify-center mb-2">
              ...
              <div className="font-fell text-[0.6rem] tracking-[0.25em] text-amber opacity-50 uppercase">
                your latest scribe
              </div>
              <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[rgba(201,124,42,0.25)] to-transparent" />
            </div>
